...
export const fetchQuiz = async (categoryId: number, amount: number): Promise<{ response_code: number; results: Question[] }> => {
    try {
        const response = await axios.get(`${API_BASE}/api.php?amount=${amount}&category=${categoryId}&difficulty=medium&type=multiple`);
